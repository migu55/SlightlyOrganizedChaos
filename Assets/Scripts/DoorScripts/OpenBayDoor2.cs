...
public class OpenBayDoor2 : MonoBehaviour, Interactable
{
    ...
    private const string openState = "Door3Open";
    private const string closeState = "Door3Close";
    ...
    public void OpenDoor()
    {
        ...
        {
            Debug.LogWarning("Animator not set on OpenBayDoor2.");
            return;
        }
...
// Use an overlap check to reliably find colliders inside the zone (more robust than testing the truck's pivot point)
    ...
    public void CloseDoor()
    {
        ...
        if (isOpen)
        {
            ...
            SpawnerZone?.GetComponent<TruckToDisplay>()?.missionDisplay?.GetComponent<MissionDisplayController>()
                ?.ClearMissionTruck();
            isOpen = false;
        }
    }
    ...
    private bool TryProcessReceiverInSpawnerZone(out int processedMissionId, bool forceSendEvenIfEmpty = false)
    {
        ...
        Collider zoneCollider = SpawnerZone == null ? null : SpawnerZone.GetComponent<Collider>();
        if (zoneCollider == null)
        {
            Debug.LogWarning("SpawnerZone does not have a Collider component on OpenBayDoor2.");
        ...
        Vector3 extents = zoneCollider.bounds.extents + pad;
        Collider[] hits = Physics.OverlapBox(zoneCollider.bounds.center, extents, zoneCollider.transform.rotation, ~0, QueryTriggerInteraction.Collide);
        Debug.Log($"OpenBayDoor2: OverlapBox found {hits.Length} colliders in SpawnerZone '{SpawnerZone.name}' (center={zoneCollider.bounds.center}, extents={zoneCollider.bounds.extents}).");
        var trucksFound = new System.Collections.Generic.HashSet<GameObject>();
        foreach (var hit in hits)
        {
            if (hit == null) continue;
            Debug.Log($"OpenBayDoor2: Overlap hit: {hit.gameObject.name} (layer={LayerMask.LayerToName(hit.gameObject.layer)})");
            Transform t = hit.transform;
            // walk up the hierarchy to find a parent tagged "Truck"
            while (t != null && !t.CompareTag("Truck")) t = t.parent;
            if (t != null && t.CompareTag("Truck"))
        ...
        if (trucksFound.Count == 0)
        {
            ...
            Debug.Log($"OpenBayDoor2: Overlap found no trucks. Falling back to collider-bounds check over {allTrucksFallback.Length} tagged Truck objects.");
            foreach (var t in allTrucksFallback)
            {
                ...
                foreach (var c in childColliders)
                {
                    ...
                    {
                        Debug.Log($"OpenBayDoor2: Fallback detected truck {t.name} via child collider {c.gameObject.name} bounds intersection.");
                        trucksFound.Add(t);
                        added = true;
                        break;
                    }
                ...
                if (!added)
                {
                    ...
                    {
                        Debug.Log($"OpenBayDoor2: Fallback detected truck {t.name} via renderer.bounds intersection.");
                        trucksFound.Add(t);
                    }
                }
            }
            ...
            if (trucksFound.Count == 0)
            {
                ...
                Debug.Log($"OpenBayDoor2: No tagged trucks found; checking {receivers.Length} TruckReceiver components in scene.");
                foreach (var rec in receivers)
                {
                    ...
                    foreach (var c in childColliders)
                    {
                        ...
                        if (c.bounds.Intersects(zoneCollider.bounds))
                        {
                            Debug.Log($"OpenBayDoor2: Fallback detected truck (via TruckReceiver) {rec.gameObject.name} collider {c.gameObject.name} bounds intersection.");
                    ...
                    if (!added)
                    {
                        ...
                        if (r != null && r.bounds.Intersects(zoneCollider.bounds))
                        {
                            Debug.Log($"OpenBayDoor2: Fallback detected truck (via TruckReceiver) {rec.gameObject.name} via renderer.bounds intersection.");
                            trucksFound.Add(rec.gameObject);
                        }
                    }
                }
        ...
        Debug.Log($"OpenBayDoor2: Trucks found in zone: {trucksFound.Count}.");
        ...
        foreach (GameObject truck in trucksFound)
        {
            ...
            if (pzt != null && receiver == null)
            {
                ...
                Debug.Log($"OpenBayDoor2: Processing send-mode truck '{truck.name}': spawnedInZone={spawned}, pendingAvailablePallets={pending}");
                ...
                if (spawned == 0 && SpawnerZone != null)
                {
                    ...
                    if (zoneCol != null)
                    {
                        ...
                        if (overlapPallets > 0)
                        {
                            Debug.Log($"OpenBayDoor2: Overlap detected {overlapPallets} pallet objects in SpawnerZone '{SpawnerZone.name}'. Treating as spawned.");
                ...
                if (spawned > 0 || pending > 0)
                {
                    ...
                    {
                        Debug.Log($"OpenBayDoor2: force deleting send-mode truck '{truck.name}' despite spawned({spawned}) or pending({pending}) pallets.");
                        Destroy(truck);
                    ...
                    Debug.Log($"OpenBayDoor2: cannot close, send-mode truck '{truck.name}' still has spawned({spawned}) or pending({pending}) pallets.");
                    return false; // block closing
                }

                // Nothing left to process on the truck -> despawn and allow close
                Debug.Log($"OpenBayDoor2: send-mode truck '{truck.name}' is empty — processing and despawning.");
                Destroy(truck);
                // Clear mission display tied to this spawner zone (if assigned)
                SpawnerZone?.GetComponent<TruckToDisplay>()?.missionDisplay?.GetComponent<MissionDisplayController>()?.ClearMissionTruck();
                return true;
            ...
            if (receiver != null)
            {
                ...
                Debug.Log($"OpenBayDoor2: Processing receive-mode truck '{truck.name}': receivedPallets={recCount}, totalBoxes={totalBoxCount}, missionId={receiver.missionId}");
                if (totalBoxCount == 0 && !forceSendEvenIfEmpty)
                {
                    SFXController.Instance.PlayClip(SFXController.Instance.errorInput);
                    Debug.Log($"OpenBayDoor2: cannot close, receive-mode truck '{truck.name}' has no received boxes.");
                    return false; // block closing
                }
                ...
                Debug.Log($"OpenBayDoor2: receive-mode truck '{truck.name}' processed: {lastReceivedPallets.Count} pallets, {lastReceivedBoxes.Count} boxes.");
                ...
                if (missionBehavior != null && receiver.missionId >= 0 && (lastReceivedBoxes.Count > 0 || forceSendEvenIfEmpty))
                {
                    ...
                    if (truckSpawner != null)
                    {
                        ...
                        {
                            Debug.LogWarning("OpenBayDoor2: truckSpawner GameObject does not have a TruckSpawnerManager component.");
                        }
                    }
                    else
                    {
                        Debug.LogWarning("OpenBayDoor2: truckSpawner reference is null; cannot decrement receiversSent.");
                    }
                    ...
                    Debug.Log($"OpenBayDoor2: sent mission {receiver.missionId} with {submittedBoxes.Count} boxes to MissionBehavior.");
                }
        ...
        // If no trucks were found or all found trucks were blocking, it's OK to close
        return true;
    }
	...
	private IEnumerator MonitorSpawnerZoneForReceiver()
	{
		while (true)
		{
			...
			// small debug
			// Debug.Log($"OpenBayDoor2: Monitor found missionId={lastDetectedMissionId} in zone '{(SpawnerZone!=null?SpawnerZone.name:"null")}'.");
		}
	}
